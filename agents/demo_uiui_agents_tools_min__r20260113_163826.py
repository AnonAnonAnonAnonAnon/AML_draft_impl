...
"""
Minimal agent demo that can see and call the local tool set (strict schema version).
"""
...
from agent_tools_min_strict__r20260113_163826 import (
    run_upsert,
    run_get,
    run_search,
    log_tail,
...
agent = Agent(
    ...
    instructions=(
        ...
        f"1) Call run_upsert(run_id='demo_tools', run_dir='{REPO_ROOT}', "
        "logs_csv='docs/draft_tool_of_agent.txt', status='demo', note='tool smoke test').\n"
        "2) Call run_get(run_id='demo_tools').\n"
        "3) Call log_tail(path='docs/draft_tool_of_agent.txt', n=20).\n"
        "4) Call resource_disk(paths_csv='.').\n"
...
def run_demo() -> str:
    prompt = "Run the tool smoke test using the steps in your instructions."
