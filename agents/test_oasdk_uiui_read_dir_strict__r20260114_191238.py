...
def _clear_proxy_env() -> None:
    for key in ("ALL_PROXY", "all_proxy"):
        if key in os.environ:
            print(f"[debug] clearing env {key}={os.environ.get(key)}")
...
print("[debug] importing agents sdk...")
...
print("[debug] agents sdk imported.")
...
def read_script_dir_files(max_bytes_per_file: int = 2000) -> str:
    ...
    global TOOL_CALLED
    TOOL_CALLED = True
    print("[debug] tool called: read_script_dir_files")
    ...
    print(f"[debug] tool returning {len(payload)} chars")
    return payload
...
def run_demo() -> None:
    ...
    print("[debug] creating AsyncOpenAI client...")
    client = AsyncOpenAI(
    ...
    print("[debug] configuring agents sdk...")
    set_default_openai_api("chat_completions")
    set_default_openai_client(client)
    set_tracing_disabled(True)
    print("[debug] agents sdk configured.")
    ...
    print("[debug] starting Runner.run_sync...")
    result = Runner.run_sync(agent, prompt)
    print("[debug] Runner.run_sync returned.")
