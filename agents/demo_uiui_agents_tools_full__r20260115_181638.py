...
"""
UIUI + OpenAI Agents SDK demo that calls the local strict-schema tools.
"""
...
MAX_TEST_CASES = None  # set to an int or use env UIUI_TOOL_DEMO_MAX_CASES
ALLOW_FALLBACK_TOOL_DISPATCH = True
...
from agent_tools_min_strict__r20260115_181638 import (
    run_upsert,
    run_get,
    run_search,
    log_tail,
...
def run_case(label: str, prompt: str, fallback_tool: str, fallback_args: dict) -> str:
    ...
    request = _extract_tool_request(result.final_output)
    if not request:
...
def run_demo() -> None:
    ...
    test_cases = [
        (
            ...
            (
                ...
                "status='demo', note='uiui tool demo'). "
    ...
    env_max = os.environ.get("UIUI_TOOL_DEMO_MAX_CASES")
    if env_max:
        try:
