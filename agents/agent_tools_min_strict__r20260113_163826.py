...
def run_get(run_id: str) -> str:
    ...
    """
    registry = _load_registry()
    return _to_json(registry.get(run_id, {}))
...
def log_tail(path: str, n: int = 200) -> str:
    ...
    if not abs_path.exists():
        return _to_json({"path": str(abs_path), "text": "", "truncated": False, "error": "not found"})
    lines = _read_lines(abs_path)
    truncated = len(lines) > n
    text = "\n".join(lines[-n:])
    return _to_json({"path": str(abs_path), "text": text, "truncated": truncated})
...
def run_log_tail(run_id: str, n: int = 200) -> str:
    ...
    """
    record = json.loads(run_get(run_id))
...
def run_log_search(
    ...
    for log_path in logs:
        hits = json.loads(log_search(log_path, pattern, context=context, max_hits=max_hits))
...
def artifact_find(run_id: str, kind: str, latest: bool = True) -> str:
    ...
    if kind in artifacts:
        return _to_json(artifacts.get(kind, []))
...
def artifact_list(run_id: str) -> str:
    ...
    result = {kind: json.loads(artifact_find(run_id, kind, latest=False)) for kind in kinds}
