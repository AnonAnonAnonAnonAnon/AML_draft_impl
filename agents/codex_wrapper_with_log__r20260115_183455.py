...
def extract_section_from_log(log_text: str, start_marker: str, end_marker: str) -> str:
    """Extract a section between markers from a codex log."""
    if start_marker not in log_text or end_marker not in log_text:
        return ""
    after_start = log_text.split(start_marker, 1)[1]
    section = after_start.split(end_marker, 1)[0]
    return section.strip("\n")


if __name__ == "__main__":
    log_path = codex_exec_to_log("List the first 5 entries in the repo root")
    print(log_path)
