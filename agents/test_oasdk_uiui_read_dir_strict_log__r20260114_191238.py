...
"""
UIUI + OpenAI Agents SDK tool-call test with file logging.
Logs progress to a local file and uses faulthandler to capture hangs.
"""
...
LOG_PATH = Path(__file__).resolve().parents[1] / "logs" / f"uiui_read_dir_debug__r{RUN_ID}.log"
TOOL_CALLED = False
...
def _clear_proxy_env() -> None:
    for key in ("ALL_PROXY", "all_proxy"):
        if key in os.environ:
            _log(f"clearing env {key}={os.environ.get(key)}")
...
def _import_agents_sdk():
    ...
    try:
        ...
        _log(f"import agents failed: {exc}")
        raise SystemExit(
            "OpenAI Agents SDK not found or failed to import. "
            "Check proxy env and installation."
        ) from exc


_log("starting script")
_log("importing agents sdk")
...
def read_script_dir_files(max_bytes_per_file: int = 2000) -> str:
    ...
    global TOOL_CALLED
    TOOL_CALLED = True
    _log("tool called: read_script_dir_files")
    ...
    _log(f"tool returning {len(payload)} chars")
    return payload
...
def run_demo() -> None:
    ...
    _log("configuring agents sdk")
    set_default_openai_api("chat_completions")
    set_default_openai_client(client)
    set_tracing_disabled(True)
    _log("agents sdk configured")
    ...
    _log("starting Runner.run_sync")
    result = Runner.run_sync(agent, prompt)
    _log("Runner.run_sync returned")

    if not TOOL_CALLED:
        _log("TOOL_NOT_CALLED: check model/tool support or UIUI tool-call compatibility.")
    _log(f"final_output: {result.final_output}")
...
if __name__ == "__main__":
    ...
    with open(LOG_PATH, "w", encoding="utf-8") as handle:
        handle.write(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] log start\n")
        handle.flush()
    faulthandler.dump_traceback_later(15, repeat=True, file=open(LOG_PATH, "a", encoding="utf-8"))
    run_demo()
