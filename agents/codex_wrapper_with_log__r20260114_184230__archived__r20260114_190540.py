...
# 归档说明（中文详注）：
# 1) 本文件是 codex_exec_to_log 的归档副本，用于在 Python 脚本里调用 codex exec，
#    并把 Codex 的所有输出统一写入日志文件，再返回该日志路径。
# 2) 日志目录默认是 docs/codex_logs/，文件名规则为 codex_exec__r<run_id>.log。
#    run_id 未传入时，自动生成当前时间戳（YYYYMMDD_HHMMSS）。
# 3) 命令构造逻辑：
#    - search=True 时追加 --search，允许模型进行网络搜索。
#    - dangerously_bypass=True 时追加 --dangerously-bypass-approvals-and-sandbox，
#      表示绕过审批与沙箱限制（完全权限）。此时 sandbox / ask_for_approval 参数不会生效。
#    - dangerously_bypass=False 时才会使用 --sandbox 与 --ask-for-approval。
#    - config / enable / disable / images / add_dir 等为可重复参数，会按传入顺序逐项追加。
#    - model / profile / oss / local_provider / full_auto / output_schema / output_last_message /
#      json_output / color 会按需追加对应开关或选项。
# 4) 日志内容结构：
#    - 头部：时间戳、repo_root、model、instruction、最终命令行、search/dangerously_bypass 标记。
#    - 中部：完整 stdout 与 stderr（不会丢弃任何输出）。
#    - 尾部：returncode，外加包含 “token” 关键词的行（如果输出中包含 token 统计信息）。
#      如果未找到 token 行，会写明未报告。
# 5) 返回值：始终返回日志路径字符串。若 check=True 且进程返回码非 0，会在写完日志后抛出异常。
# 6) 输入输出：内部强制 capture_output=True 以保证日志完整；text=True 表示以文本模式读取输出。
#    如需向 Codex 传 stdin，可使用 input_text 参数。
